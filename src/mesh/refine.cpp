...
RefineOutput refine_red(const TriMesh &coarse) {
    ...
    std::unordered_map<Edge, int, EdgeHash> edge_map;
    edge_map.reserve(edges.size() * 2 + 8);
    for (size_t i = 0; i < edges.size(); ++i) edge_map[edges[i]] = static_cast<int>(i);
    ...
    for (int t = 0; t < nt; ++t) {
        ...
        m12_[t] = np + edge_map.at(make_edge(a, b));
        m23_[t] = np + edge_map.at(make_edge(b, c));
        m31_[t] = np + edge_map.at(make_edge(c, a));
    }
    ...
    for (int t = 0; t < nt; ++t) {
        pe_triplets.emplace_back(t, t, 1.0);
        pe_triplets.emplace_back(nt + t, t, 1.0);
        pe_triplets.emplace_back(2 * nt + t, t, 1.0);
        pe_triplets.emplace_back(3 * nt + t, t, 1.0);
    }
    ...
    auto add_sub = [&](int row_off, const double sub[3][3]) {
        for (int t = 0; t < nt; ++t) {
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
                    if (sub[i][j] != 0.0) pdg_triplets.emplace_back(row_off + 3 * t + i, 3 * t + j, sub[i][j]);
                }
            }
        }
    };
...
RefineOutput refine_mesh_red(const TriMesh &initial, int nref) {
    ...
    TriMesh current = initial;
    for (int lev = 0; lev < nref; ++lev) {
        RefineOutput out = refine_red(current);
        P_node = out.P_node * P_node;
        P_elem = out.P_elem * P_elem;
        P_dg = out.P_dg * P_dg;
        current = std::move(out.mesh);
