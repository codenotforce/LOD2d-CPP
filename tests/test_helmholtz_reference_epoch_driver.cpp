...
void verify_corrector_loop_lazy_skip_and_forced_termination() {
    ...
    require(result.dual_checks == 1 && result.H_steps == 1,
            "forced termination check/commit counts are wrong");
...
void verify_level_gap_guard_respects_epoch_minimum() {
    ScriptedBackend backend;
    backend.corrector_bounds = {0.1, 0.1, 0.1, 0.1};
    backend.error_bounds = {0.3, 0.2, 0.1};
    backend.structural = {false, false, false};
    backend.reference_level_gap = 4;
    ReferenceEpochDriverConfig config = base_config();
    config.reference_refresh_level_gap = 4;
    ...
    const ReferenceEpochDriverResult result = driver.run();
    require(result.state == ReferenceEpochDriverState::WorkLimitReached,
            "level-gap cooldown trajectory did not reach its fixed horizon");
    require(result.H_steps == 3 && result.dual_checks == 0,
            "level-gap guard ignored the per-epoch minimum H-step count");
    require(action_count(result, ReferenceEpochDriverAction::RefreshReference)
                == 1,
            "level-gap cooldown did not refresh after the minimum H-step count");
}
...
int main() {
    try {
        ...
        verify_level_gap_guard_respects_epoch_minimum();
