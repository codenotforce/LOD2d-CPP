...
### Manufactured pollution experiment on 2026-07-27

The preceding Gaussian-source scan was an implementation/performance
validation, not a clean pollution test: it had no exact solution and its
coarse FEM error was nonmonotone. The missing manufactured fixed-resolution
experiment is now implemented by `bench_helmholtz_k
--source=manufactured`. It uses
...
Since `phi=phi'=0` at both endpoints, this solution satisfies
`partial_n u - i k u=0` on the whole boundary. The primary quantity is the
exact relative weighted error
...
\qquad
\|w\|_{1,k}^2
=\|\nabla w\|_{L^2(\Omega)}^2+k^2\|w\|_{L^2(\Omega)}^2.
```

To check that an observed trend is not created by the fine reference, the
benchmark also reports
...
where `u_h in V_h` is the fine P1 Galerkin solution. The standard comparison
solution is the uncorrected Galerkin solution `u_H in V_H`; the LOD result is
the two-sided Petrov-Galerkin reconstruction `u_{H,h,ell}`. Every hierarchy
is generated by global compatible NVB, so `V_H` is a subspace of `V_h`.

The main local scan fixed `kH=1`, `kh=1/8`, and
`ell=ceil(log2(k))`. Thus increasing `k` keeps the number of coarse and fine
degrees of freedom per wavelength fixed instead of improving the resolution.
The complete results are:

| `k` | `H level / h level` | `ell` | FEM `E_exact` | FEM `E_ref` | LOD `E_exact` | LOD `E_ref` | fine `E_h` |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 4  | 5 / 11 | 2 | 0.387943 | 0.384846 | 0.171861 | 0.164527 | 0.050018 |
| 8  | 7 / 13 | 3 | 0.267922 | 0.265397 | 0.061448 | 0.053413 | 0.030198 |
| 16 | 9 / 15 | 4 | 0.281191 | 0.277685 | 0.030788 | 0.019472 | 0.023711 |
| 32 | 11 / 17 | 5 | 0.437167 | 0.430860 | 0.024315 | 0.008386 | 0.022716 |

The same coarse FEM solution is independent of the auxiliary fine space.
An overlap scan with the stricter `kh=1/16` (`fine-gap=8`) reproduced its
exact errors to roundoff:

| `k` | FEM `E_exact` | LOD `E_exact` | LOD `E_ref` | fine `E_h` |
|---:|---:|---:|---:|---:|
| 4  | 0.387943 | 0.168428 | 0.166587 | 0.025015 |
| 8  | 0.267922 | 0.056428 | 0.054352 | 0.015070 |
| 16 | 0.281191 | 0.023106 | 0.019857 | 0.011741 |

At fixed `kH=1`, coarse FEM therefore has a visible pollution regime:
from `k=8` to `k=32`, `E_exact` grows by `63.2%` and `E_ref` grows by
`61.2%`; the last doubling alone raises `E_exact` by `55.5%`. Meanwhile the
fine-reference error decreases and is only `2.27%` at `k=32`, so the FEM
growth is not a deteriorating-reference artifact. The exact FEM `L2`
relative error also grows from `0.1614` at `k=8` to `0.4118` at `k=32`.

The two-sided LOD result shows the opposite behavior over the tested range.
Its exact weighted error decreases from `0.1719` to `0.0243`, and its
fine-reference error decreases from `0.1645` to `0.00839`. The stricter
overlap scan changes the LOD magnitudes, as expected because the corrector
space is `V_h`, but confirms the same decreasing trend. These data are
numerical evidence that the present LOD removes the pollution growth seen in
standard coarse P1 FEM for `k<=32`; they are not a theorem or evidence for
arbitrarily large `k`.

All rows passed `--check`. Maximum Petrov, corrector, and interpolation
constraint residuals were `1.96e-13`, `9.43e-15`, and `1.10e-15`.
The largest local case used 131,585 fine nodes, took about 5 min 12 s wall
time, and peaked at 8.35 GiB RSS. Raw CSV, command, timing, and metadata files
are stored in `results/helmholtz_pollution/` and
`results/helmholtz_pollution_gap6/`.

Reproduce the four-point scan with:

```bash
K_VALUES="4 8 16 32" KH_TARGETS="1.0" FINE_GAP=6 \
  THREADS=8 JOBS=8 bash scripts/run_helmholtz_pollution.sh
```

Use `FINE_GAP=8` for the stricter `kh=1/16` reference scan; on a memory-limited
workstation, stop at `k=16`. The runner executes one wave number per process,
supports `.done`-based resume, and records the exact command and
`/usr/bin/time -v` output for each case.

### Large-wave-number performance path on 2026-07-27

The archived `k=32`, `fine-gap=6` row spent `294.4 s` of `341.1 s` in
local correctors and another `38.7 s` in corrected-basis/coarse-factorization
construction. Fine-reference solution time was only `4.9 s`; optimizing or
omitting the reference would therefore target the wrong stage.

The current real-coefficient formulation satisfies

```math
\Psi_{\rm test}=\overline{\Psi_{\rm trial}}.
```

The model now stores and assembles the element correctors once and obtains the
test basis by coefficient-wise conjugation. Patch-level adjoint residuals are
still computed. On the same local `k=16,H=9,h=15,ell=4`, eight-thread case,
the `/usr/bin/time -v` result changed from `21.71 s / 1,629,944 KiB` to
`18.95 s / 1,396,544 KiB`: a `12.7%` wall-time reduction and `14.3%`
peak-RSS reduction. Exact, reference, and LOD errors were unchanged to the
printed precision; the corrector, constraint, and Petrov residuals remained
below `1.1e-13`.

DirectSchur was explicitly rejected as the P1 pollution default. Its
calibration took `33.65 s / 1,545,796 KiB`, versus `19.73 s /
1,634,912 KiB` for DirectSaddle. It saves little memory here and is about
`1.7x` slower, although it remains useful for the hp path.

The SparseLU wrapper additionally supports a bounded multi-slot symbolic
cache and exact numeric-factorization reuse. Numeric reuse is accepted only
when dimensions, compressed sparsity indices, and every complex matrix value
are identical. A regression test compares the reused and unreused corrector
bases. Local `k=16` pilots found that extra cache slots increased memory and
did not improve end-to-end time, so the production default remains one slot
with numeric reuse disabled. The EPYC script pilots `1/4/8` slots and
`8/16/32/64` threads before the main run rather than extrapolating this local
result.

`scripts/run_helmholtz_pollution_server.sh` adds Release/LTO compilation,
physical-core pinning, selectable NUMA interleave, pre-run memory gates,
atomic per-case CSV output, `.done` recovery, and separate `pilot`, `main`,
and `strict` stages. The staged default is `k=32,64,128`, `kH=1`,
